...
public class RankingFlushScheduler {
    ...
    public void saveDaily() {
        saveFavorite(RankingPeriod.DAILY);
        saveKeyword(RankingPeriod.DAILY);
    }
    ...
    public void saveWeekly() {
        saveFavorite(RankingPeriod.WEEKLY);
        saveKeyword(RankingPeriod.WEEKLY);
    }
    /**
    ...
    public void saveMonthly() {
        saveFavorite(RankingPeriod.MONTHLY);
        saveKeyword(RankingPeriod.MONTHLY);
    }
    ...
    private void saveFavorite(RankingPeriod period) {
        rankingCounter.favoriteCounters(period).forEach((k, v) ->
                favoriteRankingSnapshotRepository.save(new FavoriteRankingSnapshot(rankingCounter.extractProductPostId(k), period, v))
        );
    ...
    private void saveKeyword(RankingPeriod period) {
        rankingCounter.searchCounters(period).forEach((k, v) ->
                keywordRankingSnapshotRepository.save(new KeywordRankingSnapshot(rankingCounter.extractKeyword(k), period, v))
        );
