...
![프로젝트_커버_이미지_최종.png](image/프로젝트_커버_이미지_최종.png)
## "흩어진 공구를 한곳에" 
...
### 1) 검색 데이터 정합성 최적화
    문제
    • 메인 DB에서 상품 정보를 수정하거나 삭제 해도, 검색 결과에는 과거 데이터만 조회 됨
    • 실제 상품 상세 정보와 검색 창의 리스트 정보가 달라 서비스의 신뢰도가 저하되는 리스크 발생
    
    원인
    • 효율적인 검색을 위해 메인 DB와는 별개로 역인덱싱된 검색 전용 저장소를 운영
    • 두 저장소가 실시간으로 연결되어 있지 않고 원본 데이터의 변경 사항이 검색 엔진으로 자동 전파되지 않음
    
    해결
    • 상품 정보가 변경(CUD)되는 시점에 검색 엔진 API를 호출하여 검색 데이터를 즉시 갱신하도록 로직 구현
    • 검색 속도는 유지하면서 메인 DB와 검색 엔진 간의 상태를 일치시켜 실시간 최신 정보를 제공함
...
### 3) 배포 환경(JAR) 내 파일 쓰기 불가 및 영속성 이슈
    문제
    • 운영 서버(JAR) 환경에서 classpath 내 리소스 파일을 직접 수정하려 시도
    • AccessDeniedException이 발생하고, 재배포 시 데이터가 초기화되는 문제를 발견

    원인
    • JAR 내부 리소스는 빌드 후 Read-Only 상태이며, 표준 IO로는 압축 파일 내부 엔트리에 쓰기 작업이 불가능

    해결
    • 외부 경로의 bad-words.txt를 우선 참조하고, 부재 시 내부 리소스를 Fallback으로 로드
    • 모든 업데이트를 JAR 외부 물리 파일에 즉시 반영하여 배포와 무관한 영속성 확보
    • 데이터 로드 시 정규식을 적용하여 공백이나 특수문자가 포함된 변형 비속어 필터링 성능 고도화
...
### 향후 계획
#### 기술적 고도화
    • 무중단 배포를 위한 추가적인 CI/CD 자동화 계획
    • 분산 서버를 통한 안정적인 서버 환경 구축 예정
